set -m
    ...
    --bucket-ramsize 256 --enable-index-replica 1
	...
	cbdocloader -c 127.0.0.1:8091 \
	-u Administrator -p password -b $COUCHBASE_BUCKET -m 256 \
	-d /opt/couchbase/samples/travel-sample.zip
   
   
   curl -X POST -v -u $COUCHBASE_ADMINISTRATOR_USERNAME:$COUCHBASE_ADMINISTRATOR_PASSWORD http://localhost:8093/query/service -d statement=CREATE%20PRIMARY%20INDEX%20%60travel-sample-primary-index%60%20ON%20%60demobucket%60%20USING%20GSI
